...
function* getRegionsAction({ payload }) {
	...
	try {
		const { data } = yield call(
			api.request.get,
			queryBuilder(`/counts/info-counts/${id}`)
		);
...
function* SetPhoneView({ payload }) {
	try {
		yield call(
			api.request.post,
			queryBuilder(`/doctor/show-phone/${payload}`)
		);
...
function* SetVacancyPhone({ payload }) {
	try {
		yield call(
			api.request.post,
			queryBuilder(`/vacation/count-view/${payload}`)
		);
...
export default function* root() {
	yield all([
		...
		takeLatest(
			systemActions.LoadSocialSettings.TRIGGER,
			LoadSocialSettings
		),
